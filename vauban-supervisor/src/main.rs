...
const TOPOLOGY: &[PipeTopology] = &[
    ...
    // This is the only edge where audit acts as a vault CLIENT.
    PipeTopology {
        from: Service::Audit,
        to: Service::Vault,
    },
...
fn run_supervisor() -> Result<()> {
    ...
    // Auto-provision the audit WORM signing key on first boot (idempotent), so
    // every environment gets a signed tamper-evident log without a manual step.
    // MUST run before the startup loop: `audit` is the first service spawned and
    // `service_env_vars("audit")` reads the sealed key to forward it.
    ensure_audit_signing_key(&config);
...
/// Handle a Kerberos KDC FD-lease request from the sandboxed RDP proxy.
///
/// The proxy cannot `connect()` to the KDC (TCP 88) once sealed, so during
/// the CredSSP Kerberos leg each sspi `NetworkRequest` asks the supervisor
/// to open a socket to the configured KDC and hand the connected FD via
/// SCM_RIGHTS. The proxy then performs the framed Kerberos TCP I/O itself
/// — the root TCB never sees AS-REQ / TGS-REQ / ticket bytes.
///
...
///   `[auth.kerberos]` configured KDC (request carries no destination;
///   `data` is ignored),
/// - **fail-closed**: any error (Kerberos disabled, wrong caller, no
///   endpoint, connect / `send_fd` failure, timeout) yields
///   `success: false` with no FD,
/// - **caller restricted** to `proxy_rdp` (only service that speaks CredSSP
///   Kerberos).
///
/// Ordering: `send_fd` **before** `KerberosKdcResponse` (same convention as
/// `TcpConnect` / recording) so the proxy can `recv_fd_timed` without race.
fn handle_kerberos_kdc_request(
    request_id: u64,
    session_id: String,
    _data: shared::messages::SensitiveBytes,
    requesting_channel: &IpcChannel,
    requesting_service_key: &str,
    children: &HashMap<String, ChildState>,
    ...
    let fail = |error: String| {
        warn!(
            ...
            "Kerberos KDC FD lease failed: {error}"
        );
    ...
    // Only proxy-rdp may lease KDC sockets (CredSSP Kerberos path).
    if requesting_service_key != "proxy_rdp" {
        fail(format!(
            "Kerberos KDC FD lease refused for service {requesting_service_key}"
        ));
        return;
    }
    ...
    let requester_state = match children.get(requesting_service_key) {
        Some(s) => s,
        None => {
            fail(format!(
                "unknown requesting service {requesting_service_key}"
            ));
            return;
        }
    };
    let fd_socket = match &requester_state.fd_passing_socket {
        Some(sock) => sock.as_raw_fd(),
        None => {
            fail("no fd_passing socket for proxy_rdp".to_string());
            return;
        }
    };
    ...
    debug!(
        session_id = %session_id,
        kdc = %socket_addr,
        "Kerberos KDC FD leased to proxy_rdp"
    );
    ...
    let _ = requesting_channel.send(&response);
    // Keep the TcpStream alive until after send_fd (child owns a copy).
    drop(stream);
}
...
/// Validate a WORM audit segment name.
///
/// The name MUST be exactly `YYYY/MM/audit-<n>.jsonl` (4-digit year, 2-digit
/// month, decimal counter). This is the structural fix for the recording
/// path-traversal class (VAU-006) on the audit path: the audit child cannot
/// supply an arbitrary relative path, only a name matching this shape, so no
/// `..`, absolute path, or symlink-bait component can ever reach the broker.
fn validate_audit_segment_name(name: &str) -> Result<(), String> {
    if name.contains("..") || name.starts_with('/') || name.contains('\\') || name.contains('\0') {
        return Err("segment name contains a forbidden component".to_string());
    }
    let parts: Vec<&str> = name.split('/').collect();
    if parts.len() != 3 {
        return Err("segment name must be YYYY/MM/audit-<n>.jsonl".to_string());
    }
    let (year, month, file) = (parts[0], parts[1], parts[2]);
    if year.len() != 4 || !year.bytes().all(|b| b.is_ascii_digit()) {
        return Err("invalid year component".to_string());
    }
    if month.len() != 2 || !month.bytes().all(|b| b.is_ascii_digit()) {
        return Err("invalid month component".to_string());
    }
    let digits = file
        .strip_prefix("audit-")
    ...
    match digits {
        ...
        _ => Err("invalid segment file name".to_string()),
    }
}

/// Ensure the audit WORM Ed25519 signing key exists, generating it on first
/// boot so EVERY environment gets a tamper-evident (signed) WORM log out of the
/// box -- no manual `vauban-vault seal-audit-key` provisioning step.
///
/// Idempotent: a no-op when the sealed key already exists. When absent, the key
/// is generated by a one-shot `vauban-vault seal-audit-key <path>` subprocess so
/// the master-key material only ever lives in the vault process, never in the
/// long-running supervisor daemon. The path is the dedicated `[audit]` tree
/// (`signing_key_path()`), the exact location the supervisor later reads to
...
fn ensure_audit_signing_key(config: &crate::config::SupervisorConfig) {
    ...
    if sealed_path.exists() {
        debug!(path = %sealed_path.display(), "audit signing key present; skipping auto-provision");
        return;
    }

    let Some(vault_bin) = config.binary_path("vault") else {
        warn!("Cannot auto-provision audit signing key: vault binary path unknown");
        return;
    };
    ...
    if let Some(parent) = sealed_path.parent()
        && let Err(e) = std::fs::create_dir_all(parent)
    {
        warn!(path = %parent.display(), error = %e, "Failed to create audit key directory; WORM log will be unsigned (BestEffort)");
        return;
    }

    info!(
        path = %sealed_path.display(),
        "Audit signing key absent; auto-provisioning a fresh Ed25519 key via vauban-vault seal-audit-key"
    );

    match std::process::Command::new(&vault_bin)
        .arg("seal-audit-key")
        .arg(&sealed_path)
        .status()
    {
        Ok(status) if status.success() => {
            info!(path = %sealed_path.display(), "Audit signing key provisioned; WORM log will be Ed25519-sealed");
        }
        Ok(status) => {
            warn!(
                ?status,
                "vauban-vault seal-audit-key exited non-zero; WORM log will be unsigned (BestEffort)"
            );
        }
        Err(e) => {
            warn!(error = %e, "Failed to spawn vauban-vault seal-audit-key; WORM log will be unsigned (BestEffort)");
        }
    }
}

/// Handle an `AuditLogFileRequest`: open the confined append-only WORM segment
/// and hand its FD to the audit child via SCM_RIGHTS.
///
/// Confinement: the path is `<audit_base>/<segment_name>` with `segment_name`
/// validated by [`validate_audit_segment_name`]. `audit_base` is the dedicated
/// `[audit] log_path` tree (separate from session recordings). The file is
/// opened `create(true).append(true)` -- NEVER `truncate`/`O_TRUNC` -- so an
/// existing WORM log can only ever be extended, preserving the hash chain.
fn handle_audit_log_file_request(
    request_id: u64,
    segment_name: &str,
    audit_base: &str,
    requester_state: &ChildState,
    ...
    if let Err(e) = validate_audit_segment_name(segment_name) {
        error!(segment_name, error = %e, "Rejected AuditLogFileRequest (invalid segment name)");
        fail(e);
        return;
    }

    let full_path = std::path::Path::new(audit_base).join(segment_name);

    if let Some(parent) = full_path.parent()
        && let Err(e) = std::fs::create_dir_all(parent)
    {
        error!(path = %full_path.display(), error = %e, "Failed to create audit segment directory");
        fail(format!("mkdir: {e}"));
        return;
    }

    // APPEND-ONLY: create if absent, append otherwise, NEVER truncate.
        ...
        .open(&full_path)
    {
        Ok(f) => f,
        Err(e) => {
            error!(path = %full_path.display(), error = %e, "Failed to open audit segment append-only");
            fail(format!("open: {e}"));
            return;
        }
    };
    ...
    let fd_socket = match &requester_state.fd_passing_socket {
        Some(sock) => sock.as_raw_fd(),
        None => {
            error!("No FD passing socket for audit service");
            fail("no fd_passing socket".to_string());
            return;
        }
    };

    if let Err(e) = send_fd(fd_socket, file.as_raw_fd()) {
        error!(error = %e, "Failed to send audit segment fd via SCM_RIGHTS");
        fail(format!("send_fd: {e}"));
        return;
    }
...
/// KerberosKdcRequest (proxy-rdp KDC FD lease), RecordingFileRequest
/// (audit/proxies), and ACME renewal requests (web). The `mailer` config is
/// consulted by [`handle_tcp_connect_request`] when `target_service = Web`
        ...
        Ok(ready_indices) => {
            for idx in ready_indices {
                ...
                match state.channel.try_recv() {
                    ...
                    Ok(Message::AuditLogFileRequest {
                        request_id,
                        segment_name,
                    }) => {
                        debug!(
                            ...
                            "Received AuditLogFileRequest from {}",
                            service_key
                        );
                        handle_audit_log_file_request(
                            request_id,
                            &segment_name,
                            audit_log_path,
                            state,
...
mod tests {
    ...
    #[test]
    fn test_topology_audit_connections() {
        let audit_connections: Vec<_> = TOPOLOGY
            .iter()
            .filter(|conn| conn.from == Service::Audit)
            .collect();

        // Audit connects out to: Vault ONLY (unseal the Ed25519 signing-key
        // seed via VaultDecrypt{audit} at boot). Every other audit edge is
        // INCOMING (producers -> audit), never outgoing.
        assert_eq!(audit_connections.len(), 1);
        assert!(
            audit_connections.iter().any(|c| c.to == Service::Vault),
            "Audit MUST have exactly one outgoing edge, to Vault, to unseal \
             its WORM signing key at boot"
        );
    }

    // ==================== Audit WORM broker confinement (VAU-006 closure) ====

    #[test]
    fn audit_segment_name_accepts_well_formed_names() {
        for name in [
            "2026/06/audit-0.jsonl",
            "2026/06/audit-1749081600.jsonl",
            "1970/01/audit-00001.jsonl",
        ] {
            assert!(
                validate_audit_segment_name(name).is_ok(),
                "well-formed segment {name} must be accepted"
            );
        }
    }

    #[test]
    fn audit_segment_name_rejects_traversal_and_malformed() {
        // Path traversal / absolute / odd components MUST be rejected: this is
        // the structural closure of the recording path-traversal class on the
        // audit path (the audit child cannot smuggle a `..`).
        for bad in [
            "../../etc/passwd",
            "2026/06/../../../etc/passwd",
            ...
            assert!(
                ...
                "malicious/malformed segment {bad:?} MUST be rejected"
            );
        }
    }

    /// Auto-provisioning is idempotent: when the sealed key already exists,
    /// `ensure_audit_signing_key` is a no-op (it must NOT regenerate / overwrite
    ...
    fn ensure_audit_signing_key_is_noop_when_key_exists() {
        ...
        config.audit.signing_key_path = Some(key_path.to_string_lossy().into_owned());

        ensure_audit_signing_key(&config);

        // Untouched content + no `.pub` materialised (no subprocess ran).
        assert_eq!(
            std::fs::read(&key_path).unwrap(),
            b"v1:EXISTING-SEALED-KEY",
            "existing sealed key must not be overwritten"
        );
        assert!(
            !dir.join("signing_key.sealed.pub").exists(),
            "no regeneration must occur when the key already exists"
        );

        std::fs::remove_dir_all(&dir).ok();
    }

    /// Pin the auto-provisioning wiring + its security property: the supervisor
    /// generates the audit key via a ONE-SHOT `vauban-vault seal-audit-key`
    /// subprocess (so the master key never enters the supervisor daemon), and
    /// the call happens before the service startup loop (so `audit` -- spawned
    /// first -- finds the sealed key).
    #[test]
    fn auto_provision_audit_key_wired_before_startup_and_uses_subprocess() {
        let source = supervisor_prod_source();

        let call_idx = source
            .find("ensure_audit_signing_key(&config);")
            .expect("supervisor must call ensure_audit_signing_key(&config) at boot");
        let loop_idx = source
            .find("for service_key in config.startup_order()")
            .expect("supervisor must spawn services via the startup_order loop");
        assert!(
            call_idx < loop_idx,
            "ensure_audit_signing_key must run BEFORE the startup loop (audit is spawned first)"
        );

        let fn_idx = source
            .find("fn ensure_audit_signing_key")
            .expect("ensure_audit_signing_key must exist");
        let fn_src = &source[fn_idx..];
        assert!(
            fn_src.contains(".arg(\"seal-audit-key\")"),
            "auto-provision must invoke the vault `seal-audit-key` subcommand"
        );
        assert!(
            fn_src.contains("std::process::Command::new"),
            "auto-provision must use a one-shot subprocess (master key stays in the vault)"
        );

        // Security: the long-running supervisor daemon must NEVER load the
        // master key in-process. Provisioning delegates to the vault binary.
        assert!(
            !source.contains("MasterKey::from_file"),
            "the supervisor daemon must not read the vault master key directly"
        );
    }
    ...
    /// The Kerberos KDC FD broker must stay SSRF-safe and fail-closed: it
    /// connects to the SUPERVISOR-OWNED endpoint (`kerberos.endpoint()`),
    /// gates on `kerberos.allows()`, passes the socket via SCM_RIGHTS, and
    /// never derives the destination from the request (no host; `data`
    /// unused). Root must not perform Kerberos payload I/O.
    #[test]
    fn test_kerberos_kdc_fd_broker_is_ssrf_safe_and_fail_closed() {
        let source = supervisor_prod_source();
        let handler = source
        ...
        let handler_end = handler
            .find("\n/// Handle a RecordingFileRequest")
            .or_else(|| handler.find("\nfn handle_recording_file_request"))
            .unwrap_or(handler.len().min(4000));
        let handler = &handler[..handler_end];
        assert!(
            handler.contains("kerberos.allows()") && handler.contains("kerberos.endpoint()"),
            "the KDC FD broker MUST gate on kerberos.allows()/endpoint() (supervisor-owned)"
        );
        assert!(
            !handler.contains("SessionToken::verify_bytes"),
            "the KDC FD broker is token-less; it MUST NOT verify a SessionToken"
        );
        assert!(
            handler.contains("send_fd("),
            "the KDC FD broker MUST hand the connected socket via send_fd / SCM_RIGHTS"
        );
        assert!(
            handler.contains("proxy_rdp"),
            "the KDC FD broker MUST restrict callers to proxy_rdp"
        );
        // Payload I/O must live in the proxy, not the root TCB.
        assert!(
            !handler.contains("write_all(") && !handler.contains("read_exact("),
            "the KDC FD broker MUST NOT perform Kerberos payload I/O (write_all/read_exact)"
        );
        assert!(
            !handler.contains("MAX_KDC_REPLY"),
            "MAX_KDC_REPLY belongs on the proxy I/O path, not the supervisor broker"
        );
    }
