...
/// Admin view session item -- includes the username of the session owner.
#[derive(Debug, Clone)]
pub struct AdminAuthSessionItem {
    pub uuid: Uuid,
    pub username: String,
...
#[derive(Template)]
#[template(path = "accounts/admin_session_list.html")]
pub struct AdminSessionListTemplate {
    pub title: String,
    pub user: Option<UserContext>,
    pub vauban: VaubanConfig,
    pub messages: Vec<FlashMessage>,
    ...
    pub sidebar_content:
        Option<crate::templates::partials::sidebar_content::SidebarContentTemplate>,
    pub header_user: Option<crate::templates::base::UserContext>,
    pub sessions: Vec<AdminAuthSessionItem>,
}
...
mod tests {
    ...
    fn create_admin_test_session() -> AdminAuthSessionItem {
        AdminAuthSessionItem {
            uuid: Uuid::new_v4(),
            username: "testuser".to_string(),
            user_uuid: Uuid::new_v4().to_string(),
            ip_address: "10.0.0.1".to_string(),
            device_info: "Firefox on Linux".to_string(),
            last_activity: Utc::now(),
            created_at: Utc::now(),
            is_expired: false,
            is_current: false,
    ...
    #[test]
    fn test_admin_session_item_age_display() {
        let session = create_admin_test_session();
        assert_eq!(session.age_display(), "Just now");
    }

    #[test]
    fn test_admin_session_item_last_activity_display() {
        let session = create_admin_test_session();
        assert_eq!(session.last_activity_display(), "Just now");
    }
    ...
    fn test_admin_session_list_template_creation() {
        let template = AdminSessionListTemplate {
            ...
            sidebar_content: None,
            header_user: None,
            sessions: vec![create_admin_test_session()],
        };
        assert_eq!(template.sessions.len(), 1);
        assert_eq!(template.sessions[0].username, "testuser");
    }
}
