...
mod tests {
    ...
    async fn test_send_personalized_to_nonexistent_user() {
        ...
        // Should not panic
        registry.send_personalized("nonexistent", |_| "test".to_string()).await;
    }
    ...
    async fn test_send_personalized_different_messages() {
        ...
        registry.send_personalized("user-1", |hash| {
            if hash == "token-AAA" {
    ...
    async fn test_receiver_channel_capacity() {
        ...
        for i in 0..10 {
            registry.send_personalized("user-1", |_| format!("msg-{}", i)).await;
        }
    ...
    async fn test_concurrent_registrations() {
        ...
        let handle1 = tokio::spawn(async move {
            for i in 0..10 {
                let (_id, _rx) = registry_clone.register("user-1", format!("hash-{}", i)).await;
            }
        });
        ...
        let handle2 = tokio::spawn(async move {
            for i in 10..20 {
                let (_id, _rx) = registry_clone2.register("user-1", format!("hash-{}", i)).await;
            }
        });
    ...
    async fn test_send_personalized_receiver_dropped() {
        ...
        registry.send_personalized("user-1", |_| "test message".to_string()).await;
    }
    ...
    async fn test_send_personalized_some_receivers_dropped() {
        ...
        registry.send_personalized("user-1", |hash| format!("msg for {}", hash)).await;
    ...
    async fn test_many_users() {
        ...
        let mut receivers = Vec::new();
        for i in 0..50 {
            let (_id, rx) = registry.register(&format!("user-{}", i), "hash".to_string()).await;
            receivers.push(rx);
        }
