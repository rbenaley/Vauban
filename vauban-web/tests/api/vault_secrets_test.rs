...
use crate::fixtures::{
    add_secret_to_secret_group, add_user_to_vauban_group, all_secrets_group_id, create_admin_user,
    create_real_api_key, create_test_secret_access_rule, create_test_secret_group,
...
/// contains `secret_id`. Returns the raw `secrets`-scoped API key.
async fn seed_covered_user(
    ...
    create_test_secret_access_rule(conn, &ug, sg_id, true, None, None).await;
...
/// A user covered by an active rule sees exactly the granted secret in
/// the list, and the value endpoint round-trips the exact plaintext.
#[tokio::test]
#[serial]
...
async fn value_404_body(app: &TestApp, raw_key: &str, uuid_str: &str) -> String {
    let response = app
        .server
...
async fn all_denials_collapse_to_byte_identical_404() {
    ...
    create_test_secret_access_rule(&mut conn, &ug, sg2, false, None, None).await;
    ...
    create_test_secret_access_rule(&mut conn, &ug, sg4, true, None, None).await;

    // 5. Unknown (random) UUID. 6. Malformed UUID.
    let unknown_uuid = Uuid::new_v4().to_string();

    let bodies = [
        value_404_body(app, &raw_key, &no_rule_uuid.to_string()).await,
        value_404_body(app, &raw_key, &inactive_rule_uuid.to_string()).await,
        value_404_body(app, &raw_key, &expired_rule_uuid.to_string()).await,
        value_404_body(app, &raw_key, &inactive_secret_uuid.to_string()).await,
        value_404_body(app, &raw_key, &unknown_uuid).await,
        value_404_body(app, &raw_key, "not-a-uuid").await,
    ];
...
async fn virtual_all_secrets_rule_covers_fresh_secret() {
    ...
    create_test_secret_access_rule(&mut conn, &ug, virtual_id, true, None, None).await;
...
/// A superuser WITHOUT a covering rule gets 404 on `/value`: secrets
/// have no `read_all`-style bypass, the rule is the only path in.
#[tokio::test]
#[serial]
...
async fn secrets_key_isolated_from_hierarchy_zones() {
    ...
    // Vault zone: scope gate passes, list succeeds (empty is fine).
    let response = app
        .server
        .get("/api/v1/vault/secrets")
        .add_header(header::AUTHORIZATION, app.api_key_header(&raw_key))
