...
use crate::fixtures::{create_simple_user, create_simple_ssh_asset, create_test_session, unique_name};
...
async fn test_websocket_dashboard_endpoint_exists() {
    ...
    let _token = app.generate_test_token(
        &user_uuid.to_string(),
        &username,
        true,
        true,
...
async fn test_websocket_session_endpoint_exists() {
    ...
    assert!(session_id > 0, "Session should be created for WebSocket test");
}
...
async fn test_broadcast_to_user_channel() {
    ...
    assert!(received.is_ok(), "Should receive user message within timeout");
}
...
async fn test_broadcast_channel_isolation() {
    ...
    assert!(received.is_err(), "Stats channel should not receive sessions message");
}
...
async fn test_broadcast_multiple_subscribers() {
    ...
    // Send one message
    let msg = vauban_web::services::broadcast::WsMessage::new("ws-activity", "activity update".to_string());
    ...
    assert!(recv1.is_ok() && recv1.unwrap().is_ok(), "Receiver 1 should get message");
    assert!(recv2.is_ok() && recv2.unwrap().is_ok(), "Receiver 2 should get message");
    assert!(recv3.is_ok() && recv3.unwrap().is_ok(), "Receiver 3 should get message");
}
...
async fn test_user_connection_registry() {
    ...
    assert_eq!(count_after, 0, "User should have 0 connections after unregister");
}
...
async fn test_user_multiple_connections() {
    ...
    let (conn1, _rx1) = app.user_connections.register(&user_uuid, "hash1".to_string()).await;
    let (conn2, _rx2) = app.user_connections.register(&user_uuid, "hash2".to_string()).await;
    let (conn3, _rx3) = app.user_connections.register(&user_uuid, "hash3".to_string()).await;
...
async fn test_total_connections() {
    ...
    let (_conn1, _rx1) = app.user_connections.register(&user1, "hash1".to_string()).await;
    let (_conn2, _rx2) = app.user_connections.register(&user1, "hash2".to_string()).await;
    let (_conn3, _rx3) = app.user_connections.register(&user2, "hash3".to_string()).await;
    
    // Total should be 3
    let total = app.user_connections.total_connections().await;
    assert!(total >= 3, "Total connections should be at least 3, got {}", total);
}
