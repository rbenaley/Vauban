//! Correlated IPC client core (AsyncFd `try_io` pump + RAII pending GC).
//!
//! # Consumers
//!
//! - **vauban-web** AsyncFd peers (access, auth, vault, audit, proxy-ssh,
//!   proxy-rdp, proxy-iacs) — full [`CorrelatedIpcCore`] + `process_loop`.
//! - **`shared::access_guard::RbacClient`** — same AsyncFd core for the
//!   proxy-side AccessGuard re-check.
//! - **`SupervisorClient`** (vauban-web) — uses [`PendingGuard`] /
//!   [`CorrelatedIpcCore::insert_pending`] / [`CorrelatedIpcCore::alloc_id`]
//!   pattern only; keeps its sync `poll` thread + SCM_RIGHTS (not
//!   `process_loop`).
//!
//! # Invariants
//!
...
//!
//! Architecture review §4.2 / reco §10.7; alignment campaign 0.9.31.
...
impl CorrelatedIpcCore {
        ...
        loop {
            ...
            loop {
                let try_result = guard.try_io(|_| match self.channel.try_recv() {
                    Ok(msg) => Ok(msg),
                    Err(crate::ipc::IpcError::Io(ref e))
                        if e.kind() == io::ErrorKind::WouldBlock =>
                    {
                        Err(io::Error::new(io::ErrorKind::WouldBlock, "would block"))
                    }
                    Err(crate::ipc::IpcError::ConnectionClosed) => Err(io::Error::new(
    ...
    pub fn recv_blocking(&self) -> Result<Message, CorrelatedIpcError> {
        self.channel.recv().map_err(|e| match e {
            crate::ipc::IpcError::ConnectionClosed => CorrelatedIpcError::ConnectionClosed,
...
mod tests {
    use super::*;
    use crate::messages::IacsTunnelSnapshotEntry;
    use std::sync::Arc;
    use std::thread;
    use std::time::Duration;
