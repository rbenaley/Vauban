...
|----------|-------------|
| [Privilege Separation Architecture](docs/technical/Vauban_Privsep_Architecture_EN(1.3).md) | Process model, IPC protocol, Capsicum sandboxing, supervisor design |
...
| [LDAPS Auth Architecture](docs/technical/Vauban_LDAPS_Auth_Architecture_EN(1.1).md) | Directory-backed login, LDAPS bind via vauban-auth, JIT provisioning, User Group aggregation (`resolve` / `static` / `match` mapping file), anti-downgrade |
