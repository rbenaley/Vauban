...
**Changes since 1.0:** new Section 11 documenting **Vault Secrets**, the
organisational secrets manager shipped in v0.9.0 (admin store + read-only
M2M retrieval API), its triple access-rule model, the asset-provenance
verification pipeline added in v0.9.1 (source-IP matching + active host
identity challenge), and the honest HTTP status semantics of v0.9.2. The
per-peer capability matrix (4.8) and key hierarchy (3.2) now include the
dedicated `secrets` key domain.
