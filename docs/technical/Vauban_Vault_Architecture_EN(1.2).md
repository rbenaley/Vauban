...
> Supersedes
> [Vauban_Vault_Architecture_EN(1.1).md](Vauban_Vault_Architecture_EN(1.1).md).

**Changes since 1.1:** Section 7.2 documents the shared
`shared::vault_envelope::is_vault_envelope` shape predicate (parse-only,
single definition) used by web / supervisor / vault aliases. The previous
inline snippet was slightly more permissive than the production grammar;
...
11. [Vault Secrets: Organisational Secrets Manager](#11-vault-secrets-organisational-secrets-manager)
...
│
├── HKDF-SHA3-256(info="vauban-secrets-v1")       → AES-256-GCM key for organisational vault secrets
│
└── (future domains derived the same way)
```

Each **domain** (e.g. `mfa`, `credentials`, `audit`, `secrets`) has its own keyring with independently versioned derived keys. Compromising one domain's ciphertext provides zero information about another domain. The `audit` domain seals the WORM log's Ed25519 signing-key seed and is decryptable only by the `audit` peer (see the per-peer matrix in 4.8). The `secrets` domain protects the organisational Vault Secrets store (Section 11) and is reachable only by the `web` peer.
...
- `web`: `Encrypt{credentials, mfa, secrets}`, `Decrypt{credentials, secrets}`,
  `MfaGenerate`, `MfaVerify`, `MfaGetSecret`. The `secrets` domain backs the
  organisational Vault Secrets store (Section 11); no other peer may touch it.
...
The `audit` and `secrets` domains are additional keyrings (`mfa`,
`credentials`, `audit`, `secrets`) derived the same way (HKDF-SHA3-256 context
separation): compromising one keyring's ciphertext yields zero information
about the others. Domain separation also means a `credentials` ciphertext can
never be opened through the vault-secrets path (and vice versa), even if a
handler bug routes a ciphertext to the wrong surface.
...
The shape predicate lives once in `shared::vault_envelope` (parse-only; no
crypto, no base64 validation) so web / supervisor / vault cannot drift:

```rust
// shared::vault_envelope — single definition
pub fn is_vault_envelope(value: &str) -> bool {
    // Encrypted values: "v{digits}:{non_empty_payload}"
    // Does NOT validate base64 or decrypt.
    ...
    if colon_pos < 2 || colon_pos + 1 >= value.len() {
        return false;
    }
...
Handlers call thin aliases (`is_encrypted` / `is_encrypted_mfa_secret`) that
delegate to `is_vault_envelope` before deciding whether to call `VaultDecrypt`:
...
> **Status (v0.9.x):** implemented, with a deliberately narrower surface than
> sketched below. The shipped design is a **read-only retrieval API** (no
> encrypt/decrypt-as-a-service endpoints) backed by an organisational secrets
> store; see Section 11 for the actual architecture.
...
## 11. Vault Secrets: Organisational Secrets Manager

### 11.1 Overview

**Vault Secrets** (introduced in v0.9.0, beta) turns Vauban into a lightweight
organisational secrets manager: administrators create and rotate named
secrets through the web UI, and third-party applications retrieve them
**read-only** through the M2M API. It is the concrete realisation of the
"Phase 3" roadmap item (Section 10.3), with a deliberately narrower surface:
there is no encrypt/decrypt-as-a-service endpoint; the API only ever
*returns* values that administrators stored.

The vault process itself (`vauban-vault`) required almost no change: a fourth
key domain, `secrets`, was added to the keyring and granted to the `web` peer
only (Section 4.8). Everything else -- storage, grouping, authorization,
provenance, audit -- lives in the higher layers, consistent with the
"pure cryptographic service" boundary of Section 10.2.

```mermaid
flowchart LR
    Admin["Administrator<br/>(web UI, Casbin-gated)"]
    App["Third-party app<br/>(M2M, scoped API key)"]
    Web["vauban-web"]
    Access["vauban-access<br/>(authorization oracle)"]
    Vault["vauban-vault<br/>(domain 'secrets')"]
    DB[("PostgreSQL<br/>ciphertexts + rules")]

    Admin -->|"create / rotate / group<br/>(value write-only)"| Web
    App -->|"GET list / metadata / value"| Web
    Web -->|"evaluate secret access"| Access
    Web -->|"VaultEncrypt / VaultDecrypt<br/>domain='secrets'"| Vault
    Web -->|ciphertexts| DB
```

### 11.2 Roles and Surfaces

| Surface | Who | Capabilities |
|---------|-----|--------------|
| Web admin zone (`/vault/secrets`) | Administrators, gated by the Casbin `vault_secrets:manage` permission | Create, rotate, deactivate, delete secrets; manage secret groups and access rules. The secret **value is write-only**: it can be set and rotated, never displayed back in the UI. |
| M2M API (`/api/v1/vault/secrets`) | Third-party applications holding an API key with the dedicated `secrets` scope | List authorized secrets, read metadata, read the decrypted value. Strictly read-only. |

The `secrets` API-key scope is **isolated**: a key scoped for session
management or asset administration cannot call the Vault Secrets endpoints,
and vice versa. Each secret carries a monotonically increasing `version`
counter (incremented on every rotation) so consumers can detect a rotation
from metadata alone, without downloading the value.

### 11.3 Data Model

Four tables mirror the asset access-rule machinery, kept parallel to (and
fully independent from) the PAM session tables:

| Table | Role |
|-------|------|
| `vault_secrets` | Org-owned secrets. The value is a vault ciphertext envelope (`vN:...`, domain `secrets`); plaintext never reaches the database. |
| `secret_groups` | Groups of secrets, including a virtual singleton "All secrets" group (kind `all`), mirror of the "All assets" group. |
| `secret_secret_groups` | M:N membership junction. |
| `secret_access_rules` | The access rules themselves (see 11.4). |

Deletion is hard (no tombstones): traceability lives in the WORM audit log.
Database triggers protect the virtual group exactly like on the asset side
(no explicit membership, no rename/retype, no deletion).

### 11.4 Authorization: the Triple Rule

`vauban-access` is the **single authorization oracle**: `vauban-web` never
evaluates a rule locally, it asks the access service over IPC. Since v0.9.1 a
secret access rule is a **triple**:

```
(user group, secret group, asset group)
```

- **user group** -- the Vauban group of the *user who owns the API key*.
  API keys are personal; revoking a user or moving them out of the group
  instantly revokes the application's access.
- **secret group** -- which secrets the rule opens (possibly the virtual
  "All secrets" group).
- **asset group** -- *where the call is allowed to come from*: the calling
  machine must be a registered SSH/RDP asset belonging to this group
  (possibly the virtual "All assets" group). This is enforced by the
  provenance pipeline described in 11.5.

A pre-existing v0.9.0 rule (a pair, with no asset dimension) is migrated
onto the virtual "All assets" group, preserving its behavior.

### 11.5 Caller Provenance: Proving *Where* the Call Comes From

An API key proves *who* is calling; it does not prove *from where*. A stolen
key could be replayed from anywhere on the network. The provenance pipeline
(v0.9.1, `services/vault_provenance.rs`) closes this gap: before any Vault
Secrets endpoint answers -- including the list -- the caller's source IP must
be resolved to a known asset that **actively proves its identity**.

```mermaid
flowchart TB
    REQ["M2M request<br/>(valid API key, scope 'secrets')"]
    S1["Stage 1 -- IP-to-asset matching<br/>source IP vs. asset hostnames<br/>(IP literals, then DNS-resolved FQDNs)"]
    S2["Stage 2 -- active host-identity challenge<br/>connect back to source_ip:port,<br/>compare observed vs. pinned fingerprint"]
    OK["Verified asset<br/>feeds the (user, secret, asset) rule evaluation"]
    KO["403 Forbidden<br/>(provenance denied, audited)"]

    REQ --> S1
    S1 -->|"no matching asset"| KO
    S1 -->|"one or more candidates"| S2
    S2 -->|"a candidate proves its pin"| OK
    S2 -->|"no pin / challenge fails / mismatch"| KO
```

**Stage 1 -- IP-to-asset matching.** The source IP of the request is compared
against every non-deleted SSH/RDP asset. Assets whose hostname is an IP
literal are matched directly (no network I/O); assets registered by FQDN are
matched through DNS resolution, with a short per-hostname cache (60 s,
successes and failures alike) so a dead FQDN does not re-pay the lookup on
every request. Several assets may legitimately resolve to the same IP (e.g.
one SSH and one RDP service on the same machine): all of them become
candidates.

**Stage 2 -- active host-identity challenge.** An IP match alone is weak
(DHCP reuse, NAT, spoofing). Each candidate is therefore challenged in
deterministic order (ascending asset id) until one proves its identity: the
bastion connects back to **the source IP of the request** (not the asset's
registered hostname, so a poisoned DNS entry cannot redirect the challenge)
on the asset's registered port, observes the host identity presented there --
the SSH host key fingerprint for SSH assets, the server certificate SPKI
fingerprint for RDP assets -- and compares it against the fingerprint
**pinned** in the asset's connection configuration. The first candidate whose
observed fingerprint matches its pin wins and anchors the rule evaluation.

Fail-closed properties:

- an asset with **no pinned fingerprint** can never anchor provenance (it is
  skipped, never trusted on IP alone);
- a **fingerprint mismatch** is treated as a possible impersonation: it emits
  a security-critical `VaultHostIdentityMismatch` audit event and is **never
  cached**;
- a successful verification is cached in memory for 60 s, keyed by
  `(asset, pinned fingerprint)`, so steady-state M2M traffic pays the
  challenge once per minute per asset;
- any infrastructure failure (DB pool, proxy unavailable, challenge timeout)
  denies the request rather than degrading to IP-only trust.

The challenge itself is delegated to the existing sandboxed proxies
(`vauban-proxy-ssh` / `vauban-proxy-rdp`) through an injectable
`HostIdentityVerifier` seam, which keeps the pipeline deterministic under
test and reuses the supervisor's anti-SSRF connection brokering.

### 11.6 M2M API Surface and Response Semantics

```
GET /api/v1/vault/secrets                 -> list of authorized secrets (metadata)
GET /api/v1/vault/secrets/{uuid}          -> metadata of one secret
GET /api/v1/vault/secrets/{uuid}/value    -> decrypted value (critical audit)
```

Since v0.9.2 the API surface returns **honest status codes** (no
anti-enumeration on `/api/v1/*`, which is reserved for the web surface):

| Condition | Status |
|-----------|--------|
| API disabled (`api.enabled = false`) | `501 Not Implemented` |
| Missing or invalid API key | `401 Unauthorized` |
| Wrong scope, provenance denied, or no matching access rule | `403 Forbidden` |
| Secret does not exist or is inactive | `404 Not Found` |
| Malformed UUID | `400 Bad Request` |

Every `/api/*` response carries `Cache-Control: no-store` (M2M header
policy), so a secret value can never be persisted by an intermediary cache.
Reading a value emits a **critical** audit event; provenance denials emit a
dedicated `VaultProvenanceDenied` event. In total the feature adds a dozen
audit event types covering the admin CRUD, rule changes, and M2M reads.

### 11.7 Threat Model Additions

| Threat | Mitigation |
|--------|------------|
| Stolen API key replayed from an attacker's machine | Provenance: the caller IP must match a registered asset that proves its pinned host identity (11.5) |
| DNS poisoning of an asset's FQDN | The active challenge targets the request's source IP, never a DNS name |
| IP takeover (DHCP reuse, spoofed source) | The impostor cannot present the pinned SSH host key / RDP certificate; mismatch is audited as critical |
| Cross-domain ciphertext confusion | Dedicated `secrets` HKDF domain: a `credentials` or `mfa` ciphertext cannot be opened through the vault-secrets path (Section 3.2) |
| Secret value exposure through the admin UI | Values are write-only in the UI; only the M2M value endpoint returns plaintext, under full rule + provenance evaluation |
| Cache leakage of values | `Cache-Control: no-store` on the whole API surface |
