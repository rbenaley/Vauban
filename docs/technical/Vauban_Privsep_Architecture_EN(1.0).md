...
**Only `vauban-web` uses Tokio** because it handles HTTPS, WebSockets, and the REST API.

The other 7 services are **synchronous and minimalist**:
...
**Total: 11 pipe pairs (22 file descriptors)**
...
7. `vauban-web` - Depends on auth, access, audit
...
| Async Runtime | Tokio (web only) | Minimalism for other services |
...
sequenceDiagram
    ...
    V->>P: VaultCredentialResponse (SSH key)
    P->>Au: AuditEvent (SessionStart)
    P->>T: SSH Connect with injected key
