...
**Changes since 1.7:** sticky DB latch `proxy_sessions.recording_lossy`
(SSH/RDP). Proxies emit `Message::RecordingLossObserved` on the first
audit `try_send` full drop; vauban-web applies a monotone UPDATE. UI
shows an "Incomplete capture" badge. Process counter
`recording_try_send_full` (Lot F) is unchanged. IACS remains ack-block
(no latch). Crate version 0.9.22.
...
RecordingFileRequest {
    ...
    read_only: bool,           // true = open existing (web), false = create (audit)
}
```
...
| `vauban-audit` | No | SCM_RIGHTS write FDs |
| `vauban-web` | No | SCM_RIGHTS read-only FDs |
| `vauban-proxy-{rdp,ssh,iacs}` | No | Never -- recording data flows via IPC |
