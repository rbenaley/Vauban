...
### Protocol recognition troubleshooting

When an EWS `direct-tcpip` channel closes immediately after the
first bytes:

```text
grep 'iacs_protocol_mismatch\|iacs_protocol_unconfirmed' /var/log/vauban/vauban-proxy-iacs.log
```

| Log field | Meaning |
|-----------|---------|
| `expected = Modbus`, `detected = opcua` | Client spoke the wrong industrial protocol for the asset type |
| `iacs_protocol_unconfirmed` | No recognizable prefix within 4096 bytes / 5 s (fail-closed) |

**Authorization cross-type:** if `POST /assets/{uuid}/connect-iacs`
returns 403 despite a visible asset row, verify the access rule lists
the asset's exact `asset_type` (not just any `iacs_*` via the master
checkbox on a different type).

**Generic TCP:** assets typed `iacs_tcp` bypass the wire gate entirely.

Local lint before deploy:
...
| 0.7.11 | 2026-05-16 | Feature: IACS tunnels now appear on `/sessions/active` alongside SSH/RDP with the same indicator columns (badge "IACS" in amber, user -> asset, endpoint hostname, source IP, connected_at, duration, Disconnect button). Two seams: (1) the IPC pump (`vauban-web/src/ipc/proxy_iacs.rs`) now persists `proxy_sessions.status = 'tunnel_active'` (resp. `terminated`) on `IacsTunnelStatusUpdate` (resp. `IacsTunnelClosed`), anchors `connected_at` (resp. `disconnected_at`), and overwrites `client_ip` with the EWS peer IP when reported; (2) every active-list query site now filters with `status.eq_any(["active", "tunnel_active"])`. The Disconnect button dispatches `Message::IacsTunnelTerminate` via `state.proxy_iacs.terminate_tunnel(...)` (the legacy in-process registry stays only as a dev / test fall-through). Pinned by `iacs_tunnel_active_surfaces_on_active_sessions_page`, `every_active_list_query_site_includes_tunnel_active_status`, `terminate_session_handler_dispatches_iacs_via_proxy_iacs_ipc_first` and the IPC suite at `tests/ipc/iacs_lifecycle_persistence_test.rs`. |
| 0.7.12 | 2026-05-17 | Fix (FreeBSD): `vauban-proxy-iacs` was crash-looping at boot with `Failed to load/generate IACS sshd host key: Not permitted in capability mode (os error 94)` because the russh host key was opened AFTER `cap_enter`. Two-part remediation: (1) the supervisor now pre-loads the Ed25519 host key BEFORE fork (`shared::iacs_host_key::prepare_host_key_fd`) and inherits the read-only FD to the proxy via `VAUBAN_IACS_HOST_KEY_FD`; the proxy parses the FD BEFORE entering Capsicum. Same model as the HTTPS listener and the IACS sshd port (the supervisor is the only binder of privileged ports < 1024). (2) The industrial-surface gate is now a single master switch `industrial.enabled` (default `true` in every environment). The legacy `industrial.iacs_tunnel.enabled` field is silently ignored at runtime; the supervisor logs a one-shot deprecation warning at boot when it is still present in the deployed TOML. With `industrial.enabled = false`, the supervisor does NOT spawn `vauban-proxy-iacs` and skips listener / host key pre-load entirely. Pinned by `host_key_loaded_before_capsicum_test` (proxy), `industrial_unified_gate_test` + `iacs_listener_pre_bind_test` + `iacs_host_key_prepare_test` (supervisor), and `iacs_kill_switch_test` (web). |
| 0.7.13 | 2026-05-17 | Hardening: `industrial.enabled = false` now fully hides the IACS surface across Assets and Access Rules while keeping audit / traceability / recordings visible. Four-layer defense documented in the `industrial gate hide iacs surface` plan: (L2) every list query in `handlers::web::assets`, `handlers::web::manage_assets`, `handlers::api::assets` and `handlers::api::manage_assets` adds `.filter(asset_type.ne_all(AssetType::iacs_variants()))` under `!state.config.industrial.enabled` (admin / user / API list, deleted-assets audit, search-suggestion HTMX). (L3) `AssetType::select_options(industrial_enabled)` and `filter_options(industrial_enabled)` now strip every `iacs_*` variant AND the synthetic `iacs` filter token when `false`. (L4) Defense-in-depth handler re-checks: `create_asset_web` / `api::create_asset` refuse before INSERT; `asset_detail` / `asset_edit` / `update_asset_web` / `delete_asset_web` / `api::get_asset` / `api::update_asset` collapse to 404 (anti-enumeration) when the asset is IACS and the master switch is off; `create_access_rule_web` refuses a hand-crafted `allowed_iacs = true`; `update_access_rule_web` refuses ADD but PRESERVES existing `iacs_*` protocols across no-op edits (frozen-but-preserved). (L5) Template gate: `AccessRuleCreateTemplate` and `AccessRuleEditTemplate` now carry `industrial_enabled: bool`; the IACS checkbox + helper paragraph are wrapped in `{% if industrial_enabled %}`. Audit surfaces (`/sessions`, `/audit`, recordings) intentionally have NO industrial gate and are pinned by `audit_surfaces_have_no_industrial_gate`. Pinned by `every_iacs_db_filter_is_gated_on_industrial_enabled`, `every_iacs_handler_post_re_checks_industrial_enabled`, `template_industrial_enabled_field_pinned` (cross-cutting source-grep) and the `iacs_assets_surface_e2e_test` runtime suite. |
| 0.7.14 | 2026-05-17 | Fix (FreeBSD): `vauban-proxy-iacs` crash-loop on boot. Two distinct production bugs surfaced after 0.7.12. (1) **Listener `set_nonblocking` post-Capsicum** -- the proxy called `std_listener.set_nonblocking(true)` AFTER `cap_enter`, which goes through `ioctl(FIONBIO)` on BSD targets and needs `CAP_IOCTL` rights the sandbox does not grant. Manifested as `set_nonblocking on inherited IACS listener: Capabilities insufficient (os error 93)` on the very first spawn. Fixed by setting `O_NONBLOCK` in the supervisor BEFORE `fork+execv` (the flag lives on the kernel file table entry and is inherited automatically); the proxy no longer touches `fcntl()`/`ioctl()` on the inherited listener post-sandbox. (2) **Host key FD position not rewound on respawn** -- the supervisor opens the Ed25519 host key file ONCE at boot via `prepare_host_key_fd` and inherits the FD across every spawn. The kernel keeps the file position cursor on the file table entry (shared between supervisor and forked children), so the first proxy `read_to_string` to EOF advanced the cursor; every subsequent crash-and-respawn inherited the same FD parked at EOF, read an empty buffer, and crash-looped on `Failed to read IACS host key from supervisor-provided FD: invalid host key: PEM preamble contains invalid data (NUL byte)`. Fixed by `shared::iacs_host_key::rewind_host_key_fd(fd)` (`lseek(fd, 0, SEEK_SET)`) called at every push of `("VAUBAN_IACS_HOST_KEY_FD", fd)` into `inheritable_fds` (boot startup, `respawn_service`, `kill_and_respawn` watchdog). Pinned by `rewind_host_key_fd_resets_position_after_full_read` (unit), `supervisor_rewinds_host_key_fd_between_respawns` (production-bug repro), `every_iacs_host_key_fd_push_is_preceded_by_a_rewind` (drift), `supervisor_sets_nonblocking_before_execv` and `proxy_iacs_never_calls_set_nonblocking_on_inherited_listener` (Capsicum hardening). |
| 0.7.15 | 2026-05-17 | Fix (FreeBSD, follow-up to 0.7.14): once the listener and host-key crashes were resolved, `vauban-proxy-iacs` immediately surfaced a third crash-loop with `Failed to create async supervisor channel: Capabilities insufficient (os error 93)`. Root cause: `AsyncIpcChannel::new` calls `set_nonblocking(read_fd)` (`fcntl(F_GETFL/F_SETFL)`) on the supervisor / web IPC pipe FDs, but `setup_service_sandbox_with_listeners` grants those FDs `CapRights::read_write()` which deliberately omits `CAP_FCNTL`. Pre-fix the proxy built both `AsyncIpcChannel` instances AFTER `cap_enter`, so the very first fcntl crashed with errno 93. Fix: hoist both `AsyncIpcChannel::new(supervisor_channel)` and `AsyncIpcChannel::new(web_channel)` to BEFORE the sandbox call (matching the long-standing `vauban-proxy-ssh::main` ordering); the resulting non-blocking flag lives on the kernel file table entry and survives `cap_enter` untouched, while the post-Capsicum code only needs `event` (kqueue/poll) on the FD which the cap-set DOES grant. Pinned by `iacs_async_ipc_channels_constructed_before_capsicum` (source-grep: both calls strictly before `setup_service_sandbox_with_listeners` AND each appears exactly once -- guards against accidental post-sandbox re-construction). |
| 0.7.16 | 2026-05-17 | Fix (FreeBSD, follow-up to 0.7.15): once the IPC pipes were satisfied, `nc -v <bastion>:22322` would `connect()` successfully (CAP_ACCEPT was granted), but russh would stall right before sending the `SSH-2.0-russh_0.57.0` banner -- the EWS saw an open TCP session with no protocol bytes ever delivered. Root cause: on FreeBSD, `accept()` returns a new fd that **inherits the cap rights of the listening socket** (`cap_rights_inherit()`), and `CapRights::listening_socket()` did NOT carry `CAP_READ` / `CAP_WRITE` / `CAP_GETPEERNAME`. The accepted EWS connection therefore had no read/write rights, so russh's `write_all` of the banner failed silently with errno 93. macOS does not enforce these caps, hence the platform-asymmetric symptom (banner immediate on macOS, hang on FreeBSD). Fix: add `read: true`, `write: true`, `getpeername: true` to `CapRights::listening_socket()`. The listener doesn't *itself* read/write -- these rights are there ONLY to be inherited by every `accept()`-returned child fd so the application protocol can flow. Pinned by `test_cap_rights_listening_socket` (asserts the three propagation rights are present and would have caught the production bug at unit-test time). |
| 0.7.17 | 2026-05-23 | Feature: IACS protocol recognition. Authorization binds `iacs_tunnel` to `asset.asset_type` in granting access rules. `vauban-proxy-iacs` peeks the first frames on the EWS -> asset leg and rejects foreign industrial protocols (Modbus asset cannot carry OPC UA / IEC 104 / PROFINET traffic). `iacs_tcp` stays passthrough. No command-level filtering. Battle-tested: `check_iacs_protocol_gate.sh`, `protocol_gate_adversarial_test.rs`, `iacs_protocol_auth_e2e_test.rs`. See Architecture doc §14-15. |
| 0.7.18 | 2026-05-23 | Fix: IACS protocol gate observability and classify deadline. Every fail-closed path on the EWS -> asset leg now emits `iacs_protocol_unconfirmed` or `iacs_protocol_mismatch` (structured `cause` field); idle clients hit the 5 s `CLASSIFY_TIMEOUT` via `sleep_until` in the NeedMoreData select (previously blocked forever on `read()`). Feature: IACS asset edit exposes an industrial-protocol selector and persists `asset_type` in place; edit form uses HTMX (`hx-post`) instead of inline `@submit` JS. Pinned by `protocol_gate_timeout_test.rs`, expanded `protocol_gate.rs` unit tests, and `iacs_asset_create_test.rs`. |
| 0.9.37 | 2026-08-17 | ADR 006 profiles: `iacs_enip`, `iacs_bacnet_sc`, `iacs_dnp3`, `iacs_iec61850`. Same peek-then-passthrough gate; Inspect classifies Cmd on EIP Set / DNP3 Operate / MMS Write. BACnet/SC confirms TLS only. Staging: [iacs_protocol_profiles_smoke_test.md](iacs_protocol_profiles_smoke_test.md). |
